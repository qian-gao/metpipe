...
# data.limma$trial <- factor(c(rep("A", 50), rep("B", 50), rep("C", 49)))
# data.limma$gender <- factor(c(rep("M", 15), rep("F", 15), rep("O", 20),
#                               rep("M", 15), rep("F", 15), rep("O", 20),
#                               rep("M", 15), rep("F", 15), rep("O", 19)))
#
#
# data.limma$trial <- relevel(data.limma$trial, ref = "A")
# data.limma$gender <- relevel(data.limma$gender, ref = "M")
#
# data = data.limma
# formula = "~trial*gender + age"
# interaction = c("trial","gender")
# covariates <- c("age")
# random.effect = NULL
# dv = names.mapping$"Name"
#
# result <-
#   compute_lm( data = data.limma,
#               formula = "~trial*gender + age",
#               interaction = c("trial","gender"),
#               covariates <- c("age"),
#               random.effect = NULL,
  ...
  function( data = NULL,
    ...
    if (length(interaction) > 1){
      ...
      groups.comb <-
        ...
        arrange(No, desc(across(interaction))) %>%
        group_by(No) %>%
