...
generate_mzmine2_xml <-
  function(
    ...
    modules <- c("start", "RawDataImport", "SortDataFiles")

    if (!is.null(params$P_CF_MIN_RT)) modules <- c(modules, "CropFilter")

    if (!is.null(params$P_MD_NOISE_LEVEL)) modules <- c(modules, "MassDetection")
    ...
    if (!is.null(params$P_TF_FEATURE_LIST)) modules <- c(modules, "TargetedPeakDetection")
    ...
    modules <- c(modules, "SortPeakLists")

    if (!is.null(params$P_JA_MZ_ABS_TOLERANCE)) modules <- c(modules, "JoinAligner")

    if (!is.null(params$P_DF_MZ_ABS_TOLERANCE)) modules <- c(modules, "DuplicateFilter")
    ...
    if (!is.null(params$P_GF_INTENSITY_TOLERANCE)) modules <- c(modules, "PeakFinder")

    if (!is.null(params$P_DB_DATABASE)) modules <- c(modules, "CustomDBSearch")

    if (!is.null(params$P_EX_CSV)) modules <- c(modules, "CSVExport")

    if (!is.null(params$P_EX_MZTAB)) modules <- c(modules, "MzTabExport")
